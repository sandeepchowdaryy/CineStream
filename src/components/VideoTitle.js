...
const VideoTitle = ({ title, overview , id }) => {
  ...
  const trailer = filteredData?.length ? filteredData[0] : filteredData?.results[0];
  console.log(trailer);
  const handleclick = () => {
   ...
   return (
    <div className="absolute w-full   bg-gradient-to-r  from-black  px-16 py-[24%]  text-white">
      ...
      <div className="pt-4 pb-6 flex gap-3">
        <button className="bg-white text-black text-xl py-2 px-5 rounded-md hover:opacity-80" onClick={handleclick}>
          {" "}
          ▶️ Play
        </button>
        <Link to={"/movie/"+id}>
        <button className="bg-gray-700 flex justify-center items-center gap-2 text-white text-lg py-2 px-5 rounded-md hover:opacity-80">
